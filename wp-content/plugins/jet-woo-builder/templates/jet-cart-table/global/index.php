...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		...
		<tr>
			<?php
				foreach ( $table_settings['table-items'] as $item ) {
		...
		<?php if ( $table_settings['table-components']['update-button']['visible']  || $table_settings['table-components']['coupon-form']['visible'] ) : ?>

			<tr>
				<td colspan="<?php echo count( $table_settings['table-items'] ); ?>" class="actions">

					<?php if ( wc_coupons_enabled() && $table_settings['table-components']['coupon-form']['visible'] ) { ?>
						<div class="coupon">
							<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( $table_settings['table-components']['coupon-form']['placeholder'], 'jet-woo-builder' ); ?>"/>
							<button type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( $table_settings['table-components']['coupon-form']['button-label'], 'jet-woo-builder' ); ?>"><?php esc_attr_e( $table_settings['table-components']['coupon-form']['button-label'], 'jet-woo-builder' ); ?></button>
							<?php do_action( 'woocommerce_cart_coupon' ); ?>
						</div>
					<?php } ?>

					<?php if ( $table_settings['table-components']['update-button']['visible'] ) { ?>
						<button type="submit" class="button" name="update_cart" value="<?php esc_attr_e( $table_settings['table-components']['update-button']['label'], 'jet-woo-builder' ); ?>"><?php esc_html_e( $table_settings['table-components']['update-button']['label'], 'jet-woo-builder' ); ?></button>
					<?php } ?>
