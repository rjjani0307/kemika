...
if ( ! class_exists( 'YITH_Licence' ) ) {
	...
	abstract class YITH_Licence {
		...
		public function check( $product_init, $regenerate_transient = true, $force_check = false ) {
			...
			$args = array(
                'email'       => urlencode( 'aghamahdi@gmail.com' ),
                'licence_key' => 'aghamahdi',
			...
			if ( $body && is_array( $body ) ) {
				if ( isset( $body['success'] ) && true === $body['success'] ) {
                    ...
                    $licence[ $product_id ][ 'activated' ]            = true;
                    $licence[ $product_id ][ 'licence_expires' ]      = false;
                    $licence[ $product_id ][ 'licence_next_check' ]   = time() + ( 1299909998 * HOUR_IN_SECONDS );
                    $licence[ $product_id ][ 'activation_remaining' ] = false;
                    $licence[ $product_id ][ 'activation_limit' ]     = false;
                    $licence[ $product_id ][ 'is_membership' ]        = isset( $body[ 'is_membership' ] ) ? $body[ 'is_membership' ] : true;
                    $status                                           = true;
		...
		public function get_response_code_message( $code, $args = array() ) {
			...
			$messages = array(
				...
				'101'         => __( 'Matching licence key not found. Be sure to use the email used for the order in YITH and the correct license serial number.', 'yith-plugin-upgrade-fw' ),
